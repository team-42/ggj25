...
public class TexturedEntity extends AbstractEntity implements Disposable {
    ...
    public void drawShapes(ShapeRenderer shapeRenderer, boolean debugRenderingActive) {
        if (debugRenderingActive) {
            ...
            shapeRenderer.begin(ShapeRenderer.ShapeType.Line);
            Rectangle boundingRectangle = hitboxAccurate.getBoundingRectangle();
    ...
    public boolean contains(float x, float y) {
        return hitboxAccurate.getBoundingRectangle().contains(x, y);
    }

    @Override
    public boolean contains(Entity other) {
        if (other instanceof TexturedEntity o2) {
            return  hitboxAccurate.getBoundingRectangle().overlaps(o2.hitboxAccurate.getBoundingRectangle());
        } else {
